...
- **3Dキャラクター：** `chara_point.png` を使用。`mix-blend-mode: multiply` + マスクで背景を自然に馴染ませる
...
├── chara_all.png     # 3Dキャラクター（6ポーズシート）
└── CLAUDE.md         # このファイル
```
...
**株式会社TEZU Intelligence** のコーポレートHP。
...
### セクション構成
1. **Hero** — キャッチコピー＋3Dキャラ（浮遊アニメ）
2. **Problems** — よくあるお悩み6項目
3. **Services** — 提供サービス6項目
4. **Process** — 導入の流れ4ステップ
5. **Why Us** — 選ばれる理由
6. **Profile** — 代表者プロフィール
7. **CTA** — LINE相談ボタン＋3Dキャラ

---

## 🤖 チャットボット実装方針

このプロジェクトは**チャットボット付きホームページ**を目指している。

### 推奨実装アプローチ
- **Dify / Flowise** などのノーコードAIチャットをiframeで埋め込む（最速）
- または ChatGPT API（`gpt-4o-mini`）を使ったカスタムチャットウィジェットをHTMLに直接実装
- デザインはサイトのゴールドテーマに合わせ、右下固定の吹き出しUI
